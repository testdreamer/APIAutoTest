...
cf = Conf
def test_login(UST_config):
    hp = Helper()
    username = cf.getini_by_option(UST_config,'用户名和密码','username')
    password = cf.getini_by_option(UST_config, '用户名和密码', 'password')
    url = cf.getini_by_option(UST_config, '服务器地址', 'url')
    image_verifications = sys.path[0] + '/p_udemr/image_verification/image_verifications.jpg'
